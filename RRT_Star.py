...
based on: RRT*: Incremental Sampling-based Algorithms
...
def RRT_star(x_init, K, step_size,obstacles,goal_area):
    V = [x_init] 
...
def start_generate(step_size,max_nodes,obstacles,origin=Node(5,5),goal=((8,8),0.5,0.5)):
    ...
    genStart = time.time()
    RRTSTAR = RRT_star(origin, max_nodes, step_size,obstacles,goal_area=goal)
    genEnd = time.time()
    path_to_goal, cost = least_cost(RRTSTAR[0],goal)
    RRTSTAR = (RRTSTAR[0],RRTSTAR[1],path_to_goal)
    print("Amount of nodes Expanded: ", len(RRTSTAR[0])) 
    if cost == 999:
    ...
    plot(RRTSTAR,goal,obstacles,'#00A241')
    print("")
